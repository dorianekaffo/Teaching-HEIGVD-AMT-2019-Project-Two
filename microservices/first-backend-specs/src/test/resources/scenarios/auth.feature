Feature: Test des fonctionnalités d'authentification
  ...
  Scenario: Authentification de l'utilisateur présent dans le système avec un mot de passe érroné
    Given J'ai l'adresse email "doriane.tedongmokaffo@heig-vd.ch" et le mot de passe "wrongpassword"
    When Je fais un POST vers le chemin /auth/login
    Then Je reçois une réponse de code 401

  Scenario: Authentification de l'utilisateur présent dans le système avec un email érroné
    Given J'ai l'adresse email "doriane.error@heig-vd.ch" et le mot de passe "administrator"
    When Je fais un POST vers le chemin /auth/login
    Then Je reçois une réponse de code 401
